...
class ColabRecommender:
    def __init__(self, data=None, sample_size=-1):
        if data is None:
            self.data = movie_data.MovieData()
        else:
            self.data = data
    ...
    def __get_common_rated_movies(self, user_id1, user_id2):
        common_movies = []
        for element in self.data.ratings_df.loc[self.data.ratings_df['UserID'] == user_id1]['MovieID'] \
                .drop_duplicates().tolist():
            if element in self.data.ratings_df.loc[self.data.ratings_df['UserID'] == user_id2]['MovieID'] \
                    .drop_duplicates().tolist():
    ...
    # see 'Lecture Slides Part 1', slide 33 for calculation details
    ...
    def get_knn_recommendations(self, user_id, k=5):
        ...
        rating_predictions = self.predict_ratings(user_id,
                                                  [m for m in self.movie_ids if m not in already_rated_movies], k)
    ...
    def predict_ratings(self, user_id, movies_to_rate, k=5):
        ...
        for m_id in movies_to_rate:
            ...
            # occurs if movie is not in pivot table (i.e. not in training data) --> skip
            except KeyError:
                rating_predictions.append([m_id, -1.0])
